...
class AppStarSpider(CrawlSpider):
	...
	def parse_app(self, response):	
		sel = Selector(response)
                ...
                item['version'] = sel.xpath('//*[@id="app-detail-wrap"]/div[2]/div[2]/p[1]/text()').extract()[0].replace('\t','').replace('\n','').replace('\r','')[5:]
                item['rating_star'] = ""
                ...
                item['last_update'] = sel.xpath('//*[@id="app-detail-wrap"]/div[2]/ul/li[4]/text()').extract()[0].replace('\t','').replace('\n','').replace('\r','')[5:]
                descriptions = sel.xpath('//*[@id="scrollbar1"]/div[2]/div/div/text()').extract()
                destemp=''
                for des in descriptions:
                    destemp=destemp+des+' '
                item['description']=destemp.replace('\t','').replace('\n','').replace('\r','')
                item['apk_size'] = sel.xpath('//*[@id="app-detail-wrap"]/div[2]/ul/li[1]/text()').extract()[0].replace('\t','').replace('\n','').replace('\r','')[5:]
                #获取图片地址，通过空格来分割多张图片
